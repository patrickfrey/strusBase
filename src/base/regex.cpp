	...
	:m_config(0),m_errhnd(errhnd_)
{
	...
	{
		m_errhnd->report( _TXT("error in constructor of regex search: %s"), err.what());
	}
}
...
DLL_PUBLIC RegexSearch::Match RegexSearch::find( const char* start, const char* end) const
{
	...
	{
		m_errhnd->report( _TXT("error in regex search: %s"), err.what());
		return Match();
	}
}
	...
	:m_config(0),m_errhnd(errhnd_)
{
	...
	{
		m_errhnd->report( _TXT("error in constructor of regex subst: %s"), err.what());
	}
}
...
DLL_PUBLIC bool RegexSubst::exec( std::string& out, const std::string& input) const
{
	...
	{
		m_errhnd->report( _TXT("error in regex subst: %s"), err.what());
		return false;
	}
}
